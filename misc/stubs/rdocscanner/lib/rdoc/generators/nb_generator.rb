...
require 'rdoc/nb/nb_cache'
require 'rdoc/nb/nb_reader'
...
module Generators
  ...
  class NBGenerator
    ...
    def initialize(options) #:not-new:
      @options   = options
      @nb_writer = NB::NbWriter.new(options.op_dir)
    ...
    def generate(toplevels)
      RDoc::TopLevel.all_classes_and_modules.each do |cls|
        puts "Generating cls " + cls.full_name
        process_class(cls)
        puts "Done with that generation"
      end
    end
...
#      if cls === RDoc::NormalModule
      if /RDoc::NormalModule/ =~ cls.class.to_s
# END NETBEANS MODIFICATIONS      
        cls_desc = NB::ModuleDescription.new
      else
        cls_desc = NB::ClassDescription.new
        cls_desc.superclass  = cls.superclass
      end
      cls_desc.name        = cls.name
      cls_desc.full_name   = cls.full_name
      # BEGIN NETBEANS MODIFICATIONS
      cls_desc.rdoccomment = cls.comment
      # END NETBEANS MODIFICATIONS
      cls_desc.comment     = markup(cls.comment)

      cls_desc.attributes =cls.attributes.sort.map do |a|
        # BEGIN NETBEANS MODIFICATIONS
        #NB::Attribute.new(a.name, a.rw, markup(a.comment))
        NB::Attribute.new(a.name, a.rw, markup(a.comment), a.comment)
        # END NETBEANS MODIFICATIONS
      end

      cls_desc.constants = cls.constants.map do |c|
        # BEGIN NETBEANS MODIFICATIONS
        #NB::Constant.new(c.name, c.value, markup(c.comment))
        NB::Constant.new(c.name, c.value, markup(c.comment), c.comment)
        # END NETBEANS MODIFICATIONS
      end

      cls_desc.includes = cls.includes.map do |i|
        NB::IncludedModule.new(i.name)
      end

      # BEGIN NETBEANS MODIFICATIONS
      cls_desc.requires = cls.requires.map do |i|
        NB::Requires.new(i.name)
      end
      cls_desc.in_files = cls.in_files.map do |i|
        NB::InFile.new(i.file_relative_name)
      end
      # END NETBEANS MODIFICATIONS
    ...
    def generate_method_info(cls_desc, method)
      meth_desc = NB::MethodDescription.new
      ...
      # BEGIN NETBEANS MODIFICATIONS
      meth_desc.rdoccomment = method.comment
      meth_desc.call_seq = method.call_seq
      # END NETBEANS MODIFICATIONS
      ...
      meth_desc.aliases = method.aliases.map do |a|
        NB::AliasName.new(a.name)
      end

      @nb_writer.add_method(cls_desc, meth_desc)
    end
    ...
    def update_or_replace(cls_desc)
      ...
      if @options.merge
        rdr = NB::NbReader.new(NB::NbCache.new(@options.op_dir))
      ...
      if old_cls.nil?
        ...
        @nb_writer.remove_class(cls_desc)
        @nb_writer.add_class(cls_desc)
      else
        # existing class: merge in
        old_desc = rdr.get_class(old_cls)

        old_desc.merge_in(cls_desc)
        @nb_writer.add_class(old_desc)
      end
    end
  end
end
