...
class Options
  ...
  module OptionList
    ...
    OPTION_LIST = [
      ...
      [ "--nb",            "-r",   nil,
       "generate output for use by netbeans. The files are\n" +
  ...
  def parse(argv, generators)
	...
	when "--fmt"
        ...
        when "--nb", "--ri-site", "--ri-system"
          @generator_name = "nb"
          @op_dir = case opt
                    when "--nb" then NB::Paths::HOMEDIR 
                    when "--ri-site" then NB::Paths::SITEDIR
                    when "--ri-system" then NB::Paths::SYSDIR
                    else fail opt
                    end
          setup_generator(generators)
