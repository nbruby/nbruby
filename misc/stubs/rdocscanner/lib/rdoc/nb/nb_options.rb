...
module NB
  ...
  class Options
    ...
    module OptionList
      ...
      OPTION_LIST = [
        ...
        [ "--doc-dir",      "-d",   "<dirname>",
          ...
          "specified, we search the standard rdoc/nb directories.\n" +
          "May be repeated."],
        ...
        [ "--home",         nil,    nil,
          ...
          (NB::Paths::HOMEDIR || "No ~/.rdoc found") ],

        [ "--gems",         nil,    nil,
          "Include documentation from Rubygems:\n  " +
          (NB::Paths::GEMDIRS ? "#{Gem.path}/doc/*/ri" :
                                "No Rubygems ri found.") ],
                                                           
        [ "--format",       "-f",   "<name>",
          "Format to use when displaying output:\n" +
          "   " + NB::TextFormatter.list + "\n" +
      ...
      def OptionList.usage(short_form=false)
        ...
        directories << "#{Gem.path}/doc/*/ri" if NB::Paths::GEMDIRS
    ...
    def initialize
      ...
      @width        = 72
      @formatter    = NB::TextFormatter.for("plain") 
      @list_classes = false
      @list_names   = false
    ...
    def parse(args)
      ...
      begin
        ...
        go.each do |opt, arg|
          ...
          when "--format"
            @formatter = NB::TextFormatter.for(arg)
            unless @formatter
              $stderr.print "Invalid formatter (should be one of "
              $stderr.puts NB::TextFormatter.list + ")"
              exit 1
            end
          when "--width"
            begin
    ...
    def path
      NB::Paths.path(@use_system, @use_site, @use_home, @use_gems, *@doc_dirs)
    end

    def raw_path
      NB::Paths.raw_path(@use_system, @use_site, @use_home, @use_gems,
                         *@doc_dirs)
    end
