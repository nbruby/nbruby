require 'rdoc/nb/nb_paths'
require 'rdoc/usage'
require 'rdoc/nb/nb_cache'
require 'rdoc/nb/nb_util'
require 'rdoc/nb/nb_reader'
require 'rdoc/nb/nb_formatter'
require 'rdoc/nb/nb_options'
...
class  NbDriver
  ...
  def initialize
    ...
    @nb_reader = NB::NbReader.new(NB::NbCache.new(path))
  ...
  def report_method_stuff(requested_method_name, methods)
    if methods.size == 1
      method = @nb_reader.get_method(methods[0])
      @display.display_method_info(method)
    else
      entries = methods.find_all {|m| m.name == requested_method_name}
      if entries.size == 1
        method = @nb_reader.get_method(entries[0])
        @display.display_method_info(method)
      else
  ...
  def report_class_stuff(namespaces)
    if namespaces.size == 1
      klass = @nb_reader.get_class(namespaces[0])
      @display.display_class_info(klass, @nb_reader)
    else 
#      entries = namespaces.find_all {|m| m.full_name == requested_class_name}
#      if entries.size == 1
#        klass = @nb_reader.get_class(entries[0])
#        @display.display_class_info(klass, @nb_reader)
#      else
...
#      end
  ...
  def get_info_for(arg)
    ...
    namespaces = @nb_reader.top_level_namespace
    
    for class_name in desc.class_names
      namespaces = @nb_reader.lookup_namespace_in(class_name, namespaces)
      if namespaces.empty?
        raise NbError.new("Nothing known about #{arg}")
      end
    end
    ...
    if desc.method_name.nil?
      ...
      methods = @nb_reader.find_methods(desc.method_name, 
  ...
  def process_args
    if @options.list_classes
      classes = @nb_reader.full_class_names
      @display.list_known_classes(classes)
    elsif @options.list_names
      names = @nb_reader.all_names
